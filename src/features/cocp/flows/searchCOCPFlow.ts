...
export function registerSearchCOCPFlow(router: CommandRouter) {
    ...
    router.registerCallback(/^cocp_search_page_/, async (query) => {
        const chatId = query.message!.chat.id;
        const session = getSession(chatId, 'searchCOCP') as SearchSession | undefined;
        if (!session) return;
...
async function performSearch(bot: TelegramBot, chatId: number, criteria: any, username?: string, page: number = 0) {
    try {
        ...
        if (results.length === 0) {
            ...
            if (page > 0) navButtons.push({ text: '⬅️ Back', callback_data: `cocp_search_page_${page - 1}` });
            if (offset + PAGE_SIZE < count) navButtons.push({ text: 'Next ➡️', callback_data: `cocp_search_page_${page + 1}` });

            if (navButtons.length > 0) inline_keyboard.push(navButtons);
            inline_keyboard.push([{ text: '❌ Close', callback_data: 'cocp_search_cancel' }]);
